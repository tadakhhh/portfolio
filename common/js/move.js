...
$(document).ready(function() {
    ...
    $('.gallery .link1 span').delay(1500).animate({top:170, opacity:1},'slow'); //,하고 이징아웃 이런거 가눙
    ...
    //   }
      ...
      $('.gallery .link'+cnt).find('span').delay(1500).animate({top:170, opacity:1},'slow'); //텍스트의 초기화(원위치)
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      if(onoff==false){ //중지상태냐??
            onoff=true; //동작~~
            $('.ps').html('<span class="hidden">stop</span><i class="fa-regular fa-circle-stop"></i>'); //태그가 바뀌는거니까 html을 사용
      }
